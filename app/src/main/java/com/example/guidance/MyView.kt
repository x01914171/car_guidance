...
class MyView @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        for (i in polygons) {
            ...
            path.close()
            paint.color = Color.argb(255, 253, 254, 255)
        ...
        if (event?.pointerCount!! >= 1) {
            when (event.action.and(event.actionMasked)) {
                ...
                MotionEvent.ACTION_UP -> {
                    ...
                    //判断是否进行放缩
                    if (oldEventX != -1f) {
                        //寻找点击点，即通过点击选择的开始点和目的地（当按下和抬起手指移动距离小于 3f 时，被认为进行选点）
    ...
    fun zoom(scale: Float, event: MotionEvent) {
        ...
        //计算缩放中心点平移量
        val transX = WH2xy(ox.toDouble(), oy.toDouble())[0]
        val transY = WH2xy(ox.toDouble(), oy.toDouble())[1]
        //将中心点转换到原点 按比例放缩 换回坐标
        for (pt in allPointsList) {
            pt.x = ((pt.x - transX) * scale + transX)
            pt.y = ((pt.y - transY) * scale + transY)
        }
        for (pt in points) {
            pt.x = ((pt.x - transX) * scale + transX)
            pt.y = ((pt.y - transY) * scale + transY)
        ...
        for (i in polygons) {
            for (j in 0 until i.pois.size) {
                i.pois[j].x = ((i.pois[j].x - transX) * scale + transX)
                i.pois[j].y = ((i.pois[j].y - transY) * scale + transY)
            }
        }
    }
    ...
    fun translat(x0: Float, y0: Float, x1: Float, y1: Float) {
        ...
        // x 和 y 方向上平移量
        val myTransX = p2[0] - p1[0]
        val myTransY = p2[1] - p1[1]
        //分别对数组中的点坐标进行平移
        for (pt in allPointsList) {
            pt.x = pt.x + myTransX
            pt.y = pt.y + myTransY
        ...
        for (i in polygons) {
            for (j in 0 until i.pois.size) {
                i.pois[j].x = i.pois[j].x + myTransX
                i.pois[j].y = i.pois[j].y + myTransY
            }
        }
    }
